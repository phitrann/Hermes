...
"""
Gradio 5.48.0 Chat Interface with Intelligent Component Routing
...
- Mixed content → Multiple components
"""
import gradio as gr
from gradio import ChatMessage
from typing import Iterator, Optional, List, Dict, Any, Union
...
def create_agent_chat_interface(hermes_app) -> gr.Blocks:
    """
    Create Gradio interface with intelligent component routing.
    ...
    with gr.Blocks(
        ...
        Ask natural language questions about shipments and logistics data. 
        Watch the agent reason through your query and display results intelligently.
        """,
            elem_classes="header"
        )
        # Hidden state for data loading
        ...
        with gr.Row(elem_classes="input-row"):
            ...
            data_selector = gr.Dropdown(
                choices=[(p.name, str(p)) for p in csv_files],
                label="",
        ...
        def respond(
            ...
            """
            Main chat handler with intelligent component routing.
            Yields progressively:
            1. Auto-load data if needed
            2. User message added
            3. Classification thinking
            4. Query analysis
            5. Final result with appropriate component display
            """
            if not message.strip():
                return
            # Step 0: Auto-load data on first query if not already loaded
            ...
            try:
                ...
                if components["chart_path"] and os.path.exists(components["chart_path"]):
                    try:
                        ...
                        logger.info(f"Added chart to chat: {components['chart_path']}")
                    except Exception as chart_err:
                        logger.warning(f"Failed to add chart: {chart_err}")
                # Determine component visibility and data
                show_number = components["data_type"] == "number" and components["number"] is not None
                show_dataframe = components["data_type"] == "dataframe" and components["dataframe"] is not None
                ...
                final_reasoning = (
                    f"✅ **Query Complete**\n\n"
                    f"**Intent:** {intent_str}\n"
                    f"**Data Type:** {components['data_type']}\n"
                    f"**Timestamp:** {timestamp_str}\n"
                )
                # Yield final state with all components
                yield (
                    history,
                    final_reasoning,
        ...
        for btn in [suggestion_1, suggestion_2, suggestion_3]:
            btn.click(
                handle_suggestion, btn, msg_input
            ).then(
                respond, [msg_input, chatbot, data_loaded_state, data_selector], outputs
            ).then(lambda: "", None, msg_input)
    return demo