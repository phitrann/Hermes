...
class HermesApp:
    ...
    def _handle_visualization_request(self, prompt: str, force_chart: bool = True):
        ...
        stats = self.analytics.get_summary_stats() if self.analytics else {}
        response_html = self._beautify_response(response, title="📊 Hermes Visualization", footer="Chart generated by PandasAI")
        self.chat_history.append({
            "query": prompt,
        ...
        return response_html, chart_path, stats, (self.current_df.head(10) if self.current_df is not None else None), self.chat_history
    ...
    def _handle_stats_request(self, prompt: str):
        ...
        stats = self.analytics.get_summary_stats() if self.analytics else {}
        response_html = self._beautify_response(response, title="📊 Hermes Statistics", footer=f"Summary: total {stats.get('total_shipments')}, delayed {stats.get('delayed_shipments')}")
        self.chat_history.append({
            "query": prompt,
            "response": str(response),
            "intent": "statistics",
    ...
    def _handle_general_query(self, prompt: str, force_chart: bool = False):
        ...
        response = self.smart_df.chat(full_prompt)
        stats = self.analytics.get_summary_stats() if self.analytics else {}
        response_html = self._beautify_response(response, title="🤖 Hermes AI Response", footer="Powered by PandasAI & Local LLM")
...
<div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 14px; border-radius: 10px;">
  ...
  <div style="margin-top:8px; background: rgba(255,255,255,0.04); padding:10px; border-radius:8px;">
    <strong>Model:</strong> R² {metrics['r2_score']:.3f} • RMSE {metrics['rmse']:.2f} min<br>
    <strong>Forecast:</strong> Avg delay {pred['predicted_avg_delay']:.2f} min (median {pred['predicted_median']:.2f})<br>
    <em>Period:</em> {pred['forecast_period']}
  </div>
</div>
"""
            return html
